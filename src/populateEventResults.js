...
//  console.log(total_table, 'total table')
 ...
 function pop_bottom_table() {
    ...
    clinical_title_th.textContent = "Clinical events per 10,000 individuals";
    AOM_th.textContent = "AOM (inc. Wegovy)";
 ...
 function pop_icon_table() {
    ...
    rows_bottom_title.forEach((title, index) => {
       ...
       let dagger = document.createElement("sup");
       dagger.classList.add("legal");
       dagger.innerHTML = "&dagger;";
       if (event_year_input > 1) {
          icon_note.textContent = `Events avoided per ${formattedUserInputData.num_of_individuals_input} individuals over ${event_year_input} years`;
          icon_note.append(dagger);
       } else {
         icon_note.textContent = `Events avoided per ${formattedUserInputData.num_of_individuals_input} individuals over 1 year`;
         icon_note.append(dagger);
      }
